...
class DeptureNaviController: UIViewController, UITableViewDelegate, UITableViewDataSource
{
    ...
    var table0_goDestination: [Int] = [0, 0, 0]
    var table1_goDestination: [Int] = [0, 0, 0]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView.tag == 0{
            ...
            if table0_goDestination[indexPath.row] == 1{
            ...
            if table1_goDestination[indexPath.row] == 1{
    ...
    @IBAction func best_fordest(_ sender: Any) {
        //self.sort_arriveTrain0[select_row_num].obj
        //self.sort_arriveTrain1[select_row_num].obj
        /*sort_arriveTrain0, sort_arriveTrain1を線形探索し、目的駅に到着する列車を見つける
         * (追加：最も早く到着する列車も合わせて表示する)
         *
         */
        print("searchbutton clicked")
        table0_goDestination = [0, 0, 0]
        table1_goDestination = [0, 0, 0]
        var i = 0, j = 0
        for train in self.sort_arriveTrain0{
            if i > 2 {break}
            var if_stopdest = 0
            for station in train.obj{
                if station["odpt:departureStation"] == nowstation{
                    if_stopdest = 1
                } else if station["odpt:departureStation"] == destination_station || station["odpt:arrivalStation"] == destination_station {
                    if_stopdest = if_stopdest + 1
                    break
                }
            }
            if if_stopdest == 2{
                table0_goDestination[i] = 1
            }
            i = i + 1
        }
        for train in self.sort_arriveTrain1{
            if j > 2 {break}
            var if_stopdest = 0
            for station in train.obj{
                if station["odpt:departureStation"] == nowstation{
            ...
            if if_stopdest == 2{
                table1_goDestination[j] = 1
            }
            j = j + 1
        }
        arrive_train_table0.reloadData()
        arrive_train_table1.reloadData()
